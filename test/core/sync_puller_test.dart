...
void main() {
  ...
  SyncPuller puller(FakeServer server, {int pageSize = 200}) =>
      SyncPuller(api: apiFor(server), store: store, pageSize: pageSize);

  Future<LocalRecord?> local(String type, String id) => store.getLocalRecord(
  ...
  test(
    'downloads records from the start and remembers where it got to',
    () async {
      ...
      expect(
        (event.payload['title'], event.serverVersion, event.isDirty),
        ('Sports day', 3, false),
      );
  ...
  test('next time it asks only for what is new', () async {
    ...
    final server = FakeServer(
      (r) async => jsonResponse(
        call++ == 0
            ? {
                'records': [rec('school_event', 'E-1')],
                'cursor': 5,
                'hasMore': false,
              }
            : {'records': [], 'cursor': 5, 'hasMore': false},
      ),
    );
    await puller(server).pull(teacher);
    await puller(server).pull(teacher);
    expect(server.requests.map((r) => r.url.queryParameters['since']), [
      '0',
      '5',
    ]);
  });
  ...
  test(
    'pages are followed until there are no more, saving the cursor as it goes',
    () async {
      final pages = [
        {
          'records': [rec('a', '1')],
  ...
  test('an interrupted download carries on from the last saved page', () async {
    ...
    await expectLater(
      puller(server).pull(teacher),
      throwsA(isA<SyncRetryLater>()),
    );
    expect(await local('a', '1'), isNotNull);
    expect(store.cursors['${teacher.schoolId}|${teacher.id}'], 4);

    final again = FakeServer(
      (r) async => jsonResponse({'records': [], 'cursor': 4, 'hasMore': false}),
    );
  ...
  test(
    'a changed record replaces the old copy, with its new version',
    () async {
      await store.upsertLocalRecord(
        tenantId: teacher.schoolId,
        entityType: 'a',
        entityId: '1',
        payload: {'v': 1},
  ...
  test('a record with an unsent edit on this device is left alone', () async {
    ...
    final server = FakeServer(
      (r) async => jsonResponse({
        'records': [
          rec('a', '1', version: 2, payload: {'v': 'theirs'}),
        ],
        'cursor': 2,
        'hasMore': false,
      }),
    ...
    final record = (await local('a', '1'))!;
    expect(
      (record.payload['v'], record.serverVersion, record.isDirty),
      ('mine', 1, true),
    );
  });
  ...
  test('a record deleted on the server is removed here', () async {
    ...
    final server = FakeServer(
      (r) async => jsonResponse({
        'records': [
          rec('a', '1', version: 2, deleted: true),
          rec('a', 'never-here', deleted: true),
        ],
        'cursor': 3,
        'hasMore': false,
      }),
  ...
  test('cursors are kept apart for each membership and school', () async {
    final server = FakeServer(
      (r) async => jsonResponse({'records': [], 'cursor': 8, 'hasMore': false}),
    );
    await puller(server).pull(teacher);
    expect(
  ...
  test(
    'offline and an expired sign-in mean try later, and the cursor does not move',
    () async {
      await expectLater(
        puller(
      ...
      await expectLater(
        puller(FakeServer((r) async => jsonResponse({}, 401))).pull(teacher),
        throwsA(
          isA<SyncRetryLater>().having(
            (e) => e.needsSignIn,
            'needsSignIn',
  ...
  test(
    'a school the person no longer belongs to is an error to show, not silence',
    () async {
      await expectLater(
        puller(
          FakeServer(
  ...
  test('a strange answer does not corrupt anything', () async {
    await expectLater(
      puller(FakeServer((r) async => jsonResponse({'nope': 1}))).pull(teacher),
