...
void main() {
  group('ApiConfig', () {
    test('builds addresses whatever slashes are used, and is off without a base', () {
      expect(const ApiConfig('https://x.ng/api/v1').uri('me/').toString(), 'https://x.ng/api/v1/me/');
      expect(const ApiConfig('https://x.ng/api/v1/').uri('/me/').toString(), 'https://x.ng/api/v1/me/');
      expect(const ApiConfig('https://x.ng/api/v1').uri('sync/pull/', {'since': '3'}).toString(),
  ...
  group('ApiClient', () {
    ...
    test('posts a JSON body', () async {
      ...
      expect(server.requests.single.headers['Content-Type'], contains('application/json'));
    });

    test('renews an expired token once and repeats the request', () async {
      final tokens = MemoryTokenStore()..tokens = const AuthTokens(access: 'old', refresh: 'r-1');
      final server = FakeServer((r) async {
        if (r.url.path.endsWith('auth/token/refresh/')) return jsonResponse({'access': 'new'});
        return r.headers['Authorization'] == 'Bearer new' ? jsonResponse({'ok': 1}) : jsonResponse({}, 401);
      });
    ...
    test('keeps a rotated refresh token', () async {
      final tokens = MemoryTokenStore()..tokens = const AuthTokens(access: 'old', refresh: 'r-1');
      final server = FakeServer((r) async {
        if (r.url.path.endsWith('auth/token/refresh/')) return jsonResponse({'access': 'new', 'refresh': 'r-2'});
        return r.headers['Authorization'] == 'Bearer new' ? jsonResponse({}) : jsonResponse({}, 401);
      });
      await apiFor(server, tokens: tokens).get('me/');
      expect(tokens.tokens!.refresh, 'r-2');
    });

    test('two requests that both find the token expired share one renewal', () async {
      final tokens = MemoryTokenStore()..tokens = const AuthTokens(access: 'old', refresh: 'r-1');
      final server = FakeServer((r) async {
    ...
    test('when the token cannot be renewed the sign-in is dropped', () async {
      final tokens = MemoryTokenStore()..tokens = const AuthTokens(access: 'old', refresh: 'dead');
      final server = FakeServer((r) async => jsonResponse({'detail': 'no'}, 401));
      await expectLater(apiFor(server, tokens: tokens).get('me/'), throwsA(isA<SessionExpiredException>()));
    ...
    test('with no sign-in it does not even ask the server', () async {
      ...
      await expectLater(api.get('me/'), throwsA(isA<SessionExpiredException>()));
    ...
    test('a sign-in request does not send or renew a token', () async {
      ...
      await expectLater(
        ...
        throwsA(isA<ApiException>().having((e) => e.message, 'message', 'No active account found')),
      );
      expect(server.requests.single.headers.containsKey('Authorization'), isFalse);
      expect(server.requests.length, 1);
    });

    test('no signal, a timeout and a broken server all mean try again later', () async {
      for (final failing in <Future<http.Response> Function(http.Request)>[
        ...
        await expectLater(apiFor(FakeServer(failing)).get('me/'), throwsA(isA<ApiOfflineException>()));
      }
    });

    test('a server that answers slowly past the timeout is treated as offline', () async {
      ...
      final api = ApiClient(
        config: testConfig,
        tokens: MemoryTokenStore()..tokens = const AuthTokens(access: 'a', refresh: 'r'),
    ...
    test('errors carry the servers own words and code', () async {
      final cases = <(Object, int, String, String?)>[
        ({'code': 'membership_required', 'message': 'Say which role.'}, 400, 'Say which role.', 'membership_required'),
        ({'detail': 'You do not have permission.'}, 403, 'You do not have permission.', null),
        ({'password': ['Too short.', 'Too common.']}, 400, 'Too short. Too common.', null),
        (<String, Object>{}, 404, 'That was not found.', null),
        (<String, Object>{}, 429, 'Too many attempts. Please wait a moment and try again.', null),
      ];
      for (final (data, status, message, code) in cases) {
        final error = await apiFor(FakeServer((r) async => jsonResponse(data, status))).get('x/').then<Object?>((_) => null, onError: (e) => e);
    ...
    test('a page that is not JSON does not crash', () async {
      final server = FakeServer((r) async => http.Response('<html>oops</html>', 404));
