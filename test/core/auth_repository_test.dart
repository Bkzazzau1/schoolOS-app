...
const _me = {
  ...
  'name': 'Musa Ibrahim',
  'memberships': [
    {'id': 'm-1', 'schoolId': 's-1', 'schoolName': 'BrightGate', 'role': 'teacher'},
...
void main() {
  ...
  AuthRepository repo(FakeServer server) =>
      AuthRepository(api: apiFor(server, tokens: tokens), tokens: tokens, schoolSession: session);

  test('signing in keeps the tokens and the schools the person can act in', () async {
    final server = FakeServer((r) async {
      if (r.url.path.endsWith('auth/token/')) return jsonResponse({'access': 'a-1', 'refresh': 'r-1'});
      return jsonResponse(_me);
    });
    final profile = await repo(server).signIn('  Musa@School.NG ', 'a-good-password');

    expect(body(server.to('auth/token/').single), {'email': 'musa@school.ng', 'password': 'a-good-password'});
    expect(server.to('auth/token/').single.headers.containsKey('Authorization'), isFalse);
    ...
    expect(session.memberships.map((m) => (m.id, m.role)), [('m-1', SchoolRole.teacher), ('m-2', SchoolRole.parent)]);
    expect(sessionStore.memberships.length, 2);
  });

  test('wrong details give the servers message and keep nothing', () async {
    final server = FakeServer((r) async => jsonResponse({'detail': 'No active account found with the given credentials'}, 401));
    await expectLater(
      repo(server).signIn('a@b.ng', 'wrong'),
      throwsA(isA<ApiException>().having((e) => e.message, 'message', contains('No active account'))),
    );
    expect(tokens.tokens, isNull);
    expect(session.memberships, isEmpty);
  });

  test('a role this version of the app does not know is skipped, not a crash', () async {
    final server = FakeServer((r) async => jsonResponse({
          ..._me,
          'memberships': [
            {'id': 'm-1', 'schoolId': 's-1', 'schoolName': 'BrightGate', 'role': 'teacher'},
            {'id': 'm-9', 'schoolId': 's-1', 'schoolName': 'BrightGate', 'role': 'astronaut'},
          ],
        }));
  ...
  test('offline, the last known schools stay as they were', () async {
    ...
    await expectLater(repo(server).refreshProfile(), throwsA(isA<ApiOfflineException>()));
    expect(session.memberships.map((m) => m.id), [teacher.id]);
  });

  test('a school the person lost is no longer active after the list is refreshed', () async {
    await session.setMemberships([teacher]);
    await session.selectSchool(teacher);
    tokens.tokens = const AuthTokens(access: 'a', refresh: 'r');
    final server = FakeServer((r) async => jsonResponse({..._me, 'memberships': []}));
