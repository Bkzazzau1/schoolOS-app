...
HttpSyncTransport transport(FakeServer server, {TokenStore? tokens}) =>
    HttpSyncTransport(apiFor(server, tokens: tokens));
...
void main() {
  ...
  test('a delete has no payload and a create has no base version', () async {
    final server = FakeServer(
      (r) async =>
          jsonResponse({'disposition': 'accepted', 'serverVersion': 2}),
    );
    await transport(server).pushMutation(mutation(op: SyncOperation.delete));
    await transport(
      server,
    ).pushMutation(mutation(op: SyncOperation.create, base: null));
  ...
  test('accepted carries the new version', () async {
    ...
    expect(
      (result.disposition, result.serverVersion),
      (SyncPushDisposition.accepted, 4),
    );
  });

  test(
    'a conflict and a refusal are decisions, with the servers words',
    () async {
      final conflict = await transport(
        FakeServer(
          (r) async => jsonResponse({
      ...
      expect(
        (conflict.disposition, conflict.serverVersion, conflict.message),
        (
          SyncPushDisposition.conflict,
          7,
          'This record changed on the server first.',
        ),
      );

      final rejected = await transport(
        FakeServer(
          (r) async => jsonResponse({
      ...
      expect(
        (rejected.disposition, rejected.message),
        (SyncPushDisposition.rejected, 'Only the owner can decide.'),
      );
    },
  );

  test(
    'losing access to the school is a refusal with a plain message',
    () async {
      final result = await transport(
        FakeServer((r) async => jsonResponse({'detail': 'no'}, 403)),
      ).pushMutation(mutation());
  ...
  test(
    'no signal, server trouble and an expired sign-in all mean try later, never rejected',
    () async {
      for (final server in [
        ...
        await expectLater(
          transport(server).pushMutation(mutation()),
          throwsA(
      ...
      await expectLater(
        transport(
          FakeServer((r) async => jsonResponse({}, 401)),
