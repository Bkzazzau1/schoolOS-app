...
> "Smart Money Collection: the school's own collection provider (Paystack, Monnify, Remita)" at the end of this file. It is kept as
...
## Smart Money Collection: the school's own collection provider (Paystack, Monnify, Remita)

**What it is.** A school connects its **own** Paystack, Monnify or Remita account with the credentials that provider issued to it.
...
server, and the three provider adapters written against each provider's published documentation. Pending (nothing invented): Remita's
live address until an operator configures it, Monnify's contract code and Remita's service type are only proven when the first account
is made, and a parent-facing way to supply a BVN/NIN. Without a school server the app runs in demo mode as before.
