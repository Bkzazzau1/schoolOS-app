...
///
/// Changes are never written over the student: they are kept as a history and
/// applied in completion order. Promotion/class change moves the visible class,
/// transfer out removes the pupil from the active roster, and Alumni marks a
/// completed graduation. A cancelled change has no effect.
List<AdministratorStudentRecord> applyLifecycle(
  ...
  final completed = records
      .where((r) => r.status == AdministratorLifecycleStatus.completed)
      .toList()
    ..sort((a, b) {
  ...
  return [
    for (final s in students)
      () {
        ...
        var status = s.status == AdministratorStudentStatus.transferPending
        ...
        for (final r in completed) {
          ...
          if (r.isTransferOut) {
            status = AdministratorStudentStatus.transferredOut;
          }
          if (r.isAlumni) {
            status = AdministratorStudentStatus.graduated;
          }
        }
        if (status == AdministratorStudentStatus.active &&
            pendingTransfer.contains(s.id)) {
...
List<AdministratorLifecycleRecord> historyOf(
  String studentId,
  List<AdministratorLifecycleRecord> records,
) =>
    [
