...
class _PrincipalApprovalsPageState extends State<PrincipalApprovalsPage> {
  static const _websiteOrder = <String, int>{
    'APR-101': 0,
    'APR-102': 1,
    'APR-103': 2,
    'APR-104': 3,
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        _snapshot = snapshot;
        _error = null;
        if (!snapshot.items.any((item) => item.id == _selectedId) && snapshot.items.isNotEmpty) {
  ...
  List<PrincipalApprovalItem> _ordered(List<PrincipalApprovalItem> source) {
    final items = [...source];
    items.sort((a, b) => (_websiteOrder[a.id] ?? 999).compareTo(_websiteOrder[b.id] ?? 999));
    return items;
  }

  List<PrincipalApprovalItem> _filtered(PrincipalApprovalsSnapshot snapshot) {
    final query = _query.trim().toLowerCase();
    return _ordered(snapshot.items).where((item) {
      final matchesFilter = _filter == 'All' || item.status.label == _filter;
      final haystack = '${item.type} ${item.title} ${item.teacher} ${item.className}'.toLowerCase();
      ...
      return Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Text('Could not load approvals.', style: TextStyle(fontWeight: FontWeight.w900)),
    ...
    return ListView(
      ...
      children: [
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          return Column(children: [queue, const SizedBox(height: 16), review]);
        }),
        const SizedBox(height: 16),
        ...
        children: [
          ConstrainedBox(
            ...
            child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('PRINCIPAL · APPROVALS', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
              SizedBox(height: 4),
              Text('Approvals', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 28)),
              SizedBox(height: 4),
              Text('Review teacher work, approve it, or return it with clear comments.'),
            ]),
          ),
...
class _Kpis extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Wrap(
        ...
        runSpacing: 10,
        children: [
          _Kpi(label: 'Pending', value: '${snapshot.pendingCount}', note: 'Needs action'),
          _Kpi(label: 'High priority', value: '${snapshot.highPriorityPendingCount}', note: 'Review first'),
          _Kpi(label: 'Approved today', value: '${snapshot.approvedCount}', note: 'Prototype count'),
          _Kpi(label: 'Returned', value: '${snapshot.returnedCount}', note: 'Needs teacher changes'),
        ],
      );
}
...
class _QueuePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Approval queue', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            const Text('Teacher work sent to school leadership.'),
            const SizedBox(height: 12),
            TextField(
              decoration: const InputDecoration(prefixIcon: Icon(Icons.search_rounded), hintText: 'Search teacher, class or work...', border: OutlineInputBorder()),
            ...
            DropdownButtonFormField<String>(
              ...
              items: [for (final value in principalApprovalFilters) DropdownMenuItem(value: value, child: Text(value))],
              onChanged: (value) {
            ...
            const SizedBox(height: 10),
            if (items.isEmpty)
              const Padding(padding: EdgeInsets.symmetric(vertical: 24), child: Center(child: Text('No approval items match this filter.')))
            else
              for (final item in items) ...[
                _ApprovalTile(item: item, selected: item.id == selectedId, onTap: () => onSelected(item.id)),
                const SizedBox(height: 8),
              ],
          ]),
        ),
...
class _ApprovalTile extends StatelessWidget {
  const _ApprovalTile({required this.item, required this.selected, required this.onTap});
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _Pill(text: item.priority.label, attention: item.priority == PrincipalApprovalPriority.high),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(item.type, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
                Text(item.title, style: const TextStyle(fontWeight: FontWeight.w800)),
                Text('${item.teacher} · ${item.className} · ${item.submitted}', style: Theme.of(context).textTheme.bodySmall),
              ])),
              const SizedBox(width: 8),
              _Pill(text: item.status.label, attention: item.status == PrincipalApprovalStatus.returned),
            ]),
          ),
        ),
      );
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(item.type, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
                Text(item.title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 22)),
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('SUBMISSION PREVIEW', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 11)),
                const SizedBox(height: 6),
                ...
                Wrap(
                  spacing: 10,
                  runSpacing: 10,
                  children: [for (final detail in item.details) _Detail(detail: detail)],
                ),
              ]),
            ),
            ...
            if (item.status == PrincipalApprovalStatus.approved) ...[
              const SizedBox(height: 12),
              const _Result(text: 'Approved. The teacher can see the Principal decision after synchronization.', good: true),
            ],
            if (item.status == PrincipalApprovalStatus.returned) ...[
              const SizedBox(height: 12),
              const _Result(text: 'Returned to the teacher for revision.', good: false),
            ],
            if (item.lastReviewedAt != null) ...[
              const SizedBox(height: 10),
              Text('Last decision: ${item.lastReviewedAt}', style: Theme.of(context).textTheme.bodySmall),
              if ((item.lastComment ?? '').isNotEmpty)
                Text('Comment: ${item.lastComment}', style: Theme.of(context).textTheme.bodySmall),
            ],
          ]),
        ),
      );
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(detail.value, style: const TextStyle(fontWeight: FontWeight.w800)),
          ]),
        ),
      );
}
...
class _Result extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        padding: const EdgeInsets.all(10),
        decoration: BoxDecoration(
          color: good ? Theme.of(context).colorScheme.primaryContainer : Theme.of(context).colorScheme.errorContainer,
...
class _Rules extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Approval rules', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            Wrap(spacing: 10, runSpacing: 10, children: [for (final detail in principalApprovalRules) _Detail(detail: detail)]),
            const SizedBox(height: 10),
            Text(principalApprovalAiBoundary, style: Theme.of(context).textTheme.bodySmall),
          ]),
        ),
      );
}
...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          color: attention ? Theme.of(context).colorScheme.errorContainer : Theme.of(context).colorScheme.secondaryContainer,
