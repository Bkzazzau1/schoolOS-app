...
class _PrincipalAssignmentsPageState extends State<PrincipalAssignmentsPage> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        _snapshot = snapshot;
        _error = null;
        final qualified = snapshot.teachers.where((item) => item.canTeach(_newSubject)).toList();
        if (qualified.isNotEmpty && !qualified.any((item) => item.id == _newTeacher)) {
  ...
  Future<void> _openTransfer(PrincipalTeachingAssignment assignment) async {
    ...
    final candidates = snapshot.teachers
        .where((item) => item.id != assignment.teacherId && item.canTeach(assignment.subject))
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SegmentedButton<bool>(
                      ...
                      selected: {newStaff},
                      onSelectionChanged: (value) => setDialogState(() => newStaff = value.first),
                    ),
                    const SizedBox(height: 14),
                    if (!newStaff)
                      DropdownButtonFormField<String>(
                        value: existingId.isEmpty ? null : existingId,
                        decoration: const InputDecoration(labelText: 'Receiving teacher', border: OutlineInputBorder()),
                        items: [
                          for (final teacher in candidates)
                            DropdownMenuItem(value: teacher.id, child: Text('${teacher.name} · ${teacher.weeklyPeriods} periods')),
                        ],
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(labelText: 'Department / unit', hintText: 'Optional — Pending onboarding if blank', border: OutlineInputBorder()),
                      ),
                      const SizedBox(height: 8),
                      const Text('A provisional teaching target will be created. The staff account must later be linked through onboarding.', style: TextStyle(fontSize: 12)),
                    ],
                    const SizedBox(height: 14),
                    TextField(
                      controller: reasonController,
                      maxLines: 3,
                      decoration: const InputDecoration(labelText: 'Transfer reason', hintText: 'Why is this responsibility being handed over?', border: OutlineInputBorder()),
                    ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final assignments = snapshot.assignments.where((assignment) {
          ...
          final matchesQuery = '${assignment.className} ${assignment.subject} ${teacher?.name ?? ''}'.toLowerCase().contains(_query.toLowerCase());
          final matchesClass = _classFilter == 'All classes' || assignment.className == _classFilter;
          return matchesQuery && matchesClass;
        }).toList(growable: false);
        final unassigned = principalUnassignedSubjects.where((item) => !snapshot.assignments.any((assignment) => assignment.className == item.className && assignment.subject == item.subject)).toList();
        ...
        return ListView(
          ...
          children: [
            ...
            compact
                ? Column(children: [_assignmentForm(context, qualified), const SizedBox(height: 12), _unassignedCard(context, unassigned)])
                : Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(flex: 3, child: _assignmentForm(context, qualified)), const SizedBox(width: 14), Expanded(flex: 2, child: _unassignedCard(context, unassigned))]),
            const SizedBox(height: 18),
  ...
  Widget _sectionCards(BuildContext context, bool compact) {
    ...
    final cards = data.map((item) => Card(
          elevation: 0,
          child: Padding(
            padding: const EdgeInsets.all(14),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.$1, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text(item.$2, style: const TextStyle(fontWeight: FontWeight.w900)),
              Text('${item.$3} · ${item.$4}'),
              Text(item.$5, style: Theme.of(context).textTheme.bodySmall),
              const SizedBox(height: 6),
              Text(item.$6 ? 'Your active scope' : 'Managed by ${item.$3}', style: TextStyle(fontWeight: FontWeight.w800, color: item.$6 ? Theme.of(context).colorScheme.primary : null)),
            ]),
          ),
        ));
    return compact ? Column(children: cards.toList()) : Row(children: [for (final card in cards) Expanded(child: Padding(padding: const EdgeInsets.only(right: 8), child: card))]);
  }
  ...
  Widget _assignmentForm(BuildContext context, List<PrincipalAssignmentTeacher> qualified) => Card(
        elevation: 0,
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: [for (final item in principalAssignmentClasses) DropdownMenuItem(value: item, child: Text(item))],
            ...
            DropdownButtonFormField<String>(
              ...
              items: [for (final item in principalAssignmentSubjects) DropdownMenuItem(value: item, child: Text(item))],
              onChanged: (value) {
                if (value == null) return;
                final snapshot = _snapshot!;
                final matches = snapshot.teachers.where((teacher) => teacher.canTeach(value)).toList();
                setState(() {
                  _newSubject = value;
            ...
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(
              value: qualified.any((teacher) => teacher.id == _newTeacher) ? _newTeacher : (qualified.isEmpty ? null : qualified.first.id),
              decoration: const InputDecoration(labelText: 'Qualified teacher', border: OutlineInputBorder()),
              items: [for (final teacher in qualified) DropdownMenuItem(value: teacher.id, child: Text('${teacher.name} · ${teacher.weeklyPeriods} periods'))],
              onChanged: (value) => setState(() => _newTeacher = value ?? _newTeacher),
            ),
            const SizedBox(height: 10),
            DropdownButtonFormField<int>(
              value: _periods,
              decoration: const InputDecoration(labelText: 'Periods / week', border: OutlineInputBorder()),
              items: [for (var i = 1; i <= 10; i++) DropdownMenuItem(value: i, child: Text('$i'))],
              onChanged: (value) => setState(() => _periods = value ?? _periods),
            ),
            const SizedBox(height: 12),
            FilledButton.icon(onPressed: _saving || qualified.isEmpty ? null : _addAssignment, icon: const Icon(Icons.person_add_alt_1_outlined), label: Text(_saving ? 'Saving…' : 'Assign teacher')),
          ]),
        ),
      );
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (compact)
              ...
              Row(children: [Expanded(child: _searchField()), const SizedBox(width: 8), SizedBox(width: 190, child: _classFilterField())]),
            const SizedBox(height: 12),
  ...
  Widget _searchField() => TextField(
        decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search class, subject or teacher...', border: OutlineInputBorder()),
        onChanged: (value) => setState(() => _query = value),
      );

  Widget _classFilterField() => DropdownButtonFormField<String>(
        value: _classFilter,
        decoration: const InputDecoration(labelText: 'Class', border: OutlineInputBorder()),
        items: [const DropdownMenuItem(value: 'All classes', child: Text('All classes')), for (final item in principalAssignmentClasses) DropdownMenuItem(value: item, child: Text(item))],
  ...
  Widget _assignmentRow(BuildContext context, PrincipalTeachingAssignment assignment, bool compact) {
    final teacher = _teacherById(assignment.teacherId);
    final status = teacher?.weeklyPeriods != null && teacher!.weeklyPeriods > 24 ? 'Heavy' : 'Balanced';
    final content = [
      _cell('Class', assignment.className),
      _cell('Subject', assignment.subject, sub: teacher?.department),
      _cell('Teacher', teacher?.name ?? assignment.teacherId, sub: teacher?.provisional == true ? 'Provisional · onboarding link pending' : null),
    ...
    return Container(
      ...
      child: compact
          ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [...content, const SizedBox(height: 8), Align(alignment: Alignment.centerRight, child: FilledButton.tonalIcon(onPressed: () => _openTransfer(assignment), icon: const Icon(Icons.swap_horiz_rounded), label: const Text('Transfer work')))])
          : Row(children: [for (final item in content) Expanded(child: item), const SizedBox(width: 8), FilledButton.tonalIcon(onPressed: () => _openTransfer(assignment), icon: const Icon(Icons.swap_horiz_rounded), label: const Text('Transfer'))]),
    );
  }

  Widget _cell(String label, String value, {String? sub}) => Padding(
        padding: const EdgeInsets.symmetric(vertical: 3),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700)), Text(value, style: const TextStyle(fontWeight: FontWeight.w800)), if (sub != null) Text(sub, style: const TextStyle(fontSize: 11))]),
      );
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            for (final transfer in snapshot.transfers)
              ListTile(
                ...
                subtitle: Text('${_teacherById(transfer.fromTeacherId)?.name ?? transfer.fromTeacherId} → ${_teacherById(transfer.toTeacherId)?.name ?? transfer.toTeacherId}\n${transfer.reason}\nVersion ${transfer.previousAssignmentVersion} → ${transfer.newAssignmentVersion}'),
                isThreeLine: true,
              ),
          ]),
        ),
  ...
  Widget _governance(BuildContext context) => Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            const Text('Proprietor / Authorized Owner → Creates sections & appoints leaders → Principal / Headmaster / Headmistress → Assigns teachers inside own section'),
            const SizedBox(height: 8),
            Text(principalAssignmentScopeBoundary, style: Theme.of(context).textTheme.bodySmall),
            const SizedBox(height: 8),
            Text(principalTransferPrivacyBoundary, style: Theme.of(context).textTheme.bodySmall),
