...
class _StudentWorkspacePageState extends State<StudentWorkspacePage>
  ...
  int get _remaining => _state['deadline'] == null
      ? 600
      : DateTime.parse(_state['deadline']! as String)
  ...
  Map<String, Object?> get _subjectEligibility {
    final raw = _canonicalProfile['subjectEligibility'];
    return raw is Map ? Map<String, Object?>.from(raw) : const {};
  }

  Map<String, Object?> get _academicContext {
    final raw = _canonicalProfile['currentAcademicContext'];
    return raw is Map ? Map<String, Object?>.from(raw) : const {};
  }

  List<Map<String, Object?>> get _eligibleSubjects => [
        for (final item
            in (_subjectEligibility['eligibleSubjects'] as List? ?? const []))
          if (item is Map) Map<String, Object?>.from(item),
      ];

  List<Map<String, Object?>> get _availableElectives => [
        for (final item
            in (_subjectEligibility['availableElectives'] as List? ?? const []))
          if (item is Map) Map<String, Object?>.from(item),
      ];

  List<Map<String, Object?>> get _enrollmentHistory => [
        for (final item
            in (_canonicalProfile['enrollmentHistory'] as List? ?? const []))
          if (item is Map) Map<String, Object?>.from(item),
      ];

  List<Map<String, Object?>> get _progressionHistory => [
        for (final item
            in (_canonicalProfile['progressionHistory'] as List? ?? const []))
          if (item is Map) Map<String, Object?>.from(item),
      ];

  List<Map<String, Object?>> get _tasks => [
        for (final item in (_state['tasks'] as List? ?? const []))
          if (item is Map) Map<String, Object?>.from(item),
      ];
  ...
  Future<void> _reloadWorkspaceState() async {
    try {
      ...
      // Keep explicit action errors separate from background sync refresh.
    }
  }
  ...
  Future<void> _loadCbtSets() async {
    ...
    try {
      ...
      if (mounted) {
        setState(
          () => _cbtError = 'Could not load your class CBTs. Please retry.',
        );
      }
    } finally {
  ...
  Future<void> _runCbt(
    ...
    try {
      ...
      if (mounted) {
        setState(
          () => _cbtError = 'Could not save your CBT progress. Please retry.',
        );
      }
    } finally {
  ...
  void _select(String key) {
    if (!_studentNavigation.any((item) => item.key == key) ||
        key == _activeKey) {
      return;
    }
  ...
  String _contextName(String key) {
    final raw = _academicContext[key];
    if (raw is! Map) return '';
    return Map<String, Object?>.from(raw)['name'] as String? ?? '';
  }
    ...
    return ListView(
      ...
      children: [
        ...
        _SectionCard(
          ...
          subtitle:
              '${profile['admissionNumber'] ?? '—'} · ${profile['studentId'] ?? '—'}',
          child: Wrap(
            spacing: 12,
            runSpacing: 12,
            children: [
              ...
              _InfoTile(
                ...
                value: _contextName('session').isEmpty
                    ? '—'
                    : _contextName('session'),
              ),
              _InfoTile(
                label: 'Current term',
                value: _contextName('currentTerm').isEmpty
                    ? '—'
                    : _contextName('currentTerm'),
              ),
              _InfoTile(
                label: 'Primary guardian',
                value: profile['primaryGuardian'] as String? ?? '—',
              ),
            ],
          ),
        ),
                  ...
                  children: [
                    for (final item in _enrollmentHistory)
                      ListTile(
                        ...
                        trailing: Chip(
                          label: Text(item['status'] as String? ?? 'unknown'),
                        ),
                      ),
                  ],
                ),
        ...
        _SectionCard(
          ...
          subtitle:
              'Promotion, class change, transfer and graduation decisions remain historical.',
                  ...
                  children: [
                    for (final event in _progressionHistory)
                      ListTile(
                        ...
                        subtitle: Text(
                          '${event['status'] ?? 'unknown'} · ${_dateLabel(event['completedAt'] ?? event['requestedAt'])}',
                        ),
                      ),
                  ],
                ),
  ...
  Widget _subjects() {
    final profile = _canonicalProfile;
    final sessionName = _contextName('session');
    final className = profile['className'] as String? ?? '';
    final currentTerm = _contextName('currentTerm');
    final active = profile['enrollmentActive'] == true;
    ...
    return ListView(
      ...
      children: [
        _sectionHeader(
          title: 'My subjects',
          description:
              'Your current subjects come from the curriculum attached to your active academic session and class.',
        ),
        _SectionCard(
          title: 'Current curriculum',
          subtitle:
              'Promotion automatically moves this view to the new class curriculum. Old class subjects remain historical.',
          child: Wrap(
            spacing: 12,
            runSpacing: 12,
            children: [
              _InfoTile(
                label: 'Session',
                value: sessionName.isEmpty ? '—' : sessionName,
              ),
              _InfoTile(
                label: 'Class',
              ...
              _InfoTile(
                label: 'Current term',
                value: currentTerm.isEmpty ? '—' : currentTerm,
              ),
              _InfoTile(
                label: 'Subjects taken',
                value: '${_eligibleSubjects.length}',
              ),
            ],
          ),
        ),
        const SizedBox(height: 16),
        _SectionCard(
          title: 'Subjects I am taking',
          subtitle:
              'Compulsory subjects are automatic. Electives appear here only after an explicit selection for your current enrollment.',
          child: !active
              ? const Text(
                  'You do not currently have an active enrollment, so no current subjects are available.',
                )
              : _eligibleSubjects.isEmpty
                  ? const Text(
                      'No subjects have been published for your current class yet.',
                    )
                  : Column(
                      children: [
                        for (final subject in _eligibleSubjects)
                          _SubjectTile(subject: subject, selected: true),
                      ],
                    ),
        ),
        if (_availableElectives.isNotEmpty) ...[
          const SizedBox(height: 16),
          _SectionCard(
            title: 'Available electives',
            subtitle:
                'These subjects are available in your class curriculum but are not currently part of your selected subjects.',
            child: Column(
              children: [
                for (final subject in _availableElectives)
                  _SubjectTile(subject: subject, selected: false),
              ],
            ),
          ),
        ],
  ...
  Widget _classCbts() {
    ...
    return _SectionCard(
      title: 'My class CBTs',
      subtitle:
          'Teacher-published tests available to your current class.',
      child: _cbtError != null
          ? ListTile(
              contentPadding: EdgeInsets.zero,
              title: Text(_cbtError!),
              trailing: TextButton(
                onPressed: _loadCbtSets,
      ...
      children: [
        ...
        _SectionCard(
          ...
          subtitle: !_demo
              ...
              : 'Sample questions · Demo only · Not an official exam.',
          child: !_demo
              ? const Text('Ask your teacher for a published CBT.')
              : Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    if (_state['deadline'] == null)
                      FilledButton(
                        onPressed:
                            _busy ? null : () => _run(_repository.startPractice),
                        child: const Text('Start practice'),
                      )
                    else ...[
                      Text(
                        _submitted
                            ? 'Submitted · Score: ${_state['score']}/${studentPracticeQuestions.length}'
                            : 'Time left: ${_remaining ~/ 60}:${(_remaining % 60).toString().padLeft(2, '0')}',
                        style: const TextStyle(fontWeight: FontWeight.w800),
                      ),
                      const SizedBox(height: 8),
                      for (var i = 0;
                          i < studentPracticeQuestions.length;
                          i++)
                        Container(
                          margin: const EdgeInsets.only(bottom: 10),
                          padding: const EdgeInsets.all(12),
                              ...
                              for (var j = 0;
                                ...
                                Padding(
                                  ...
                                  child: OutlinedButton(
                                    ...
                                    onPressed:
                                        _busy || _submitted || _remaining == 0
                                            ? null
                                            : () => _run(
                                                  () => _repository.answer(i, j),
                                                ),
                                    child: Text(
  ...
  Widget _planner() => ListView(
        ...
        children: [
          ...
          _SectionCard(
            title: 'Study tasks',
            subtitle: 'Add a reminder and check it off when done.',
            child: Column(
              children: [
                Row(
                  children: [
                ...
                if (_tasks.isEmpty)
                  ...
                  for (var i = 0; i < _tasks.length; i++)
                    CheckboxListTile(
                      contentPadding: EdgeInsets.zero,
                      title: Text(_tasks[i]['title'] as String? ?? 'Task'),
            ...
            child: ListView(
              children: [
                const ListTile(
                  ...
                  subtitle: Text('Learning workspace'),
                ),
                const Divider(),
...
class _SubjectTile extends StatelessWidget {
  const _SubjectTile({required this.subject, required this.selected});
  ...
  Widget build(BuildContext context) {
    final requirement = subject['requirement'] as String? ?? 'elective';
    final periods = subject['periodsPerWeek'] as int? ?? 0;
    final name = subject['name'] as String? ?? 'Subject';
    final code = subject['code'] as String? ?? '';
    final compulsory = requirement.toLowerCase() == 'compulsory';
    return ListTile(
      contentPadding: EdgeInsets.zero,
      leading: CircleAvatar(
        child: Icon(
          compulsory ? Icons.lock_outline_rounded : Icons.tune_rounded,
          size: 18,
        ),
      ),
      ...
      subtitle: Text(
        [
          ...
          if (periods > 0) '$periods periods/week',
        ].join(' · '),
      ),
      trailing: Chip(
        label: Text(
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              label,
              style: TextStyle(fontWeight: FontWeight.w800, color: onTone),
            ),
          ],
        ),
      );
